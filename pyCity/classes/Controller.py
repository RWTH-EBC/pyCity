...
class Controller(object):
    ...
    def getDemands(self, apartments):
        ...
        for apartment in apartments:
            ...
            # Get entire electrical, domestic hot water and space heating demand
    ...
    def getFlowTemperature(self, apartments):
        ...
        relevantPreviousDays = 1 # Number of previous days' weather forecast relevant to the heating curve
        numberTimesteps = relevantPreviousDays * 24 / self.environment.getTimeDiscretization() * 3600
        (tAmbientPrevious,) = self.environment.weather.getPreviousWeather(numberTimesteps=numberTimesteps, useTimesteps=True, getTAmbient=True, getQDirect=False, getQDiffuse=False, getVWind=False, getPhiAmbient=False, getPAmbient=False)
        (tAmbientForecast,) = self.environment.weather.getWeatherForecast(getTAmbient=True, getQDirect=False, getQDiffuse=False, getVWind=False, getPhiAmbient=False, getPAmbient=False)
        ...
        # Check if this flow temperature has to be increased at certain time steps due to domestic hot water
        for apartment in apartments:
