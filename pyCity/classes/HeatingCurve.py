...
class HeatingCurve(object):
    ...
    def __init__(self, environment, m=0.33, setRoom=20, setAmbient=-10, setFlow=55, setReturn=45):
        """
        Parameters
        ----------
        timer : Timer object
    ...
    def computeRequiredFlowTemperature(self, ambientTemperature, smoothingPeriod=1):
        """
        ...
        smoothingPeriod : Integer, optional
            Over how many days (not time steps!) the moving average is computed
        ...
        ambientTemperature = self.doubleExponentialMovingAverage(ambientTemperature, smoothingPeriod)
        ...
        phi[index] = (self.setRoom - ambientTemperature[index]) / (self.setRoom - self.setAmbient)
    
        # Compute flow temperature according to heating curve
        return np.power(phi, 1/(1 + self.m)) * dTmN + 0.5 * phi * dTN + self.setRoom
        

    def doubleExponentialMovingAverage(self, timeseries, smoothingPeriod=1):
        """
        This function implements the double exponential moving average (DEMA), as 
        eplained here: http://etfhq.com/blog/2010/11/17/double-and-triple-exponential-moving-average/
...
#            1 makes the "previous" observation irrelevant
        smoothingPeriod = 24 * 3600 / self.environment.getTimeDiscretization() * smoothingPeriod
