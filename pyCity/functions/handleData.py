...
def saveResult(timer, currentAttribute, totalAttribute, currentResult):
    ...
    currentPosition = timer.getCurrentTimestep()
    finalPosition = currentPosition + timer.getTimestepsUsedHorizon()
    
    # Only save the first values that are not overwritten later
    requiredResult = currentResult[0:timer.getTimestepsUsedHorizon()]
...
def saveResultInit(timer, currentAttribute, totalAttribute, currentResult):
    ...
    """
    (currentAttribute, totalAttribute) = saveResult(timer, currentAttribute, totalAttribute, currentResult)
    init_value = currentResult[timer.getTimestepsUsedHorizon()-1]
