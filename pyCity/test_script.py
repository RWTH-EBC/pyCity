...
dishwasher = classes.Dishwasher.Dishwasher(environment, 30, np.random.rand(timer.getTimestepsTotal()), 70, 0, 0, timer.getTimeDiscretization(), thermalConnection=True, pathElectric=pathDwEl, pathThermal=pathDwTh)
washingmachine = classes.Washingmachine.Washingmachine(environment, 100, np.random.rand(timer.getTimestepsTotal()), 70, 0, 0, timer.getTimeDiscretization(), thermalConnection=True, pathElectric=pathWmEl, pathThermal=pathWmTh)
...
while environment.getCurrentDay() <= 0:
    optim.optimize(building, environment)
    environment.update()
